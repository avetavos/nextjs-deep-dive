...
Next.js คือ React framework ที่จัดการ routing, rendering, การ fetch data, bundling และ optimization ให้คุณ — และโมเดลปัจจุบันของมันคือ App Router รัน component บน server โดย default

## Next.js เป็น React framework ไม่ใช่แค่ router

React เปล่า ๆ ให้แค่โมเดล component เท่านั้น ไม่มี routing ไม่มี server ไม่มี build pipeline Next.js เข้ามาเติมช่องว่างตรงนี้ มันคือชั้นที่ตัดสินว่า URL หนึ่งจะ map ไปที่โค้ดไหน โค้ดนั้นรันที่ไหน และผลลัพธ์เดินทางไปถึง browser ยังไง
...
Next.js คือ framework ที่ครอบ React ไว้ จัดการ routing, rendering, data และ bundling — และ App Router คือโมเดลปัจจุบันของมันที่สร้างบน Server Component ส่วน Pages Router เก็บไว้เพื่อการ migrate เท่านั้น
</Callout>
