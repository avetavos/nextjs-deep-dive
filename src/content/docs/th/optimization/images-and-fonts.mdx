...
`Image` คือตัวแทนแบบ drop-in ของ `<img>` ที่ optimize ให้ตอน request โดยสร้าง version ที่ resize ตามอุปกรณ์ ส่ง WebP เมื่อ browser รองรับ (และ AVIF ด้วย ถ้าคุณเพิ่มเข้าไปใน `images.formats`) และ lazy-load ทุกอย่างที่อยู่ใต้ fold ให้อัตโนมัติ
