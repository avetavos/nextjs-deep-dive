---
title: Environment และ Configuration
description: env var เป็น server-only เป็น default ส่วน NEXT_PUBLIC_ ถูก inline เข้า client bundle ลำดับการโหลด .env และของสำคัญใน next.config.ts
sidebar:
  order: 3
---

import Mermaid from '../../../../components/Mermaid.astro';
import Callout from '../../../../components/Callout.astro';
import Quiz from '../../../../components/Quiz.tsx';
import ProgressTracker from '../../../../components/ProgressTracker.tsx';

## The idea in one sentence

env var เป็น server-only เป็น default มีแค่ตัวที่ขึ้นต้นด้วย `NEXT_PUBLIC_` เท่านั้นที่ถูก inline เข้า client bundle เพราะฉะนั้น secret ห้ามมี prefix นั้นเด็ดขาด

## Server-only versus NEXT_PUBLIC_

env var ธรรมดาใช้ได้แค่ฝั่ง server พอใส่ prefix `NEXT_PUBLIC_` Next.js จะ inline ค่าของมันเข้าไปใน JavaScript ที่ส่งไป browser ตอน build มันเหมาะกับ analytics id สาธารณะ แต่หายนะถ้าเป็น API secret

```ts
// Server Component or route — both work on the server
const dbUrl = process.env.DATABASE_URL          // server-only, safe
const analyticsId = process.env.NEXT_PUBLIC_ANALYTICS_ID // also in the client bundle
```

```txt
# .env.local
DATABASE_URL="postgres://user:pass@host:5432/db"   # never prefix a secret
NEXT_PUBLIC_ANALYTICS_ID="abcdefghijk"             # safe to expose
```

เพราะค่า `NEXT_PUBLIC_` ถูกฝังตอน build การเปลี่ยนค่าแปลว่าต้อง rebuild ใหม่ มันไม่ได้อ่านตอน runtime บน browser

## .env files and load order

Next.js โหลดไฟล์ `.env` หลายตัวและตัวที่ match ก่อนชนะ สำหรับ production ลำดับคือ `.env.production.local` แล้ว `.env.local` แล้ว `.env.production` แล้ว `.env` เก็บ secret ไว้ใน `.env.local` (gitignore) แล้ว commit เฉพาะ default ที่ปลอดภัย

```txt
.env                  # committed defaults, safe values only
.env.local            # gitignored, real secrets
.env.development      # loaded during next dev
.env.production       # loaded during next build / next start
```

## next.config.ts essentials

`next.config.ts` คือที่ที่เรา config ตัว framework เอง สี่ตัวที่ใช้บ่อยสุดคือ `images.remotePatterns` (อนุญาต image host ภายนอก), `redirects`, `rewrites` และ `headers`

```ts
// next.config.ts
import type { NextConfig } from 'next'

const nextConfig: NextConfig = {
  images: {
    remotePatterns: [
      { protocol: 'https', hostname: 's3.amazonaws.com', pathname: '/my-bucket/**' },
    ],
  },
  async redirects() {
    return [{ source: '/about', destination: '/', permanent: true }]
  },
  async rewrites() {
    return [{ source: '/docs/:slug', destination: '/help/:slug' }]
  },
  async headers() {
    return [
      {
        source: '/(.*)',
        headers: [{ key: 'X-Frame-Options', value: 'DENY' }],
      },
    ]
  },
}

export default nextConfig
```

<Mermaid title="Where each env var can be read" code={diagramEnvTh} />

export const diagramEnvTh = `
graph TD
  A["process.env.SECRET"] --> B["Server only"]
  C["process.env.NEXT_PUBLIC_ID"] --> B
  C --> D["Inlined into client bundle at build time"]
`;

<Callout title="The one-line takeaway">
มีแค่ var `NEXT_PUBLIC_` เท่านั้นที่ไปถึง browser และมันถูกฝังตอน build เก็บ secret ทุกตัวไว้แบบไม่มี prefix และไว้ใน `.env.local`
</Callout>

export const quizEnvTh = [
  {
    q: 'Where is a plain (unprefixed) env var available?',
    options: [
      'Only in the browser',
      'Only on the server',
      'Both server and client',
      'Nowhere until rebuilt',
    ],
    answer: 1,
  },
  {
    q: 'What does the NEXT_PUBLIC_ prefix do?',
    options: [
      'Encrypts the value',
      'Inlines the value into the client bundle at build time',
      'Makes it read at runtime in the browser',
      'Marks it as required',
    ],
    answer: 1,
  },
  {
    q: 'Which file should hold real secrets?',
    options: [
      '.env (committed)',
      '.env.local (gitignored)',
      'next.config.ts',
      'package.json',
    ],
    answer: 1,
  },
  {
    q: 'Which next.config.ts key allows external image hosts?',
    options: [
      'images.remotePatterns',
      'redirects',
      'headers',
      'rewrites',
    ],
    answer: 0,
  },
];

<Quiz client:visible id="production-and-ecosystem/environment-and-configuration" questions={quizEnvTh} />

<ProgressTracker client:visible id="production-and-ecosystem/environment-and-configuration" />
