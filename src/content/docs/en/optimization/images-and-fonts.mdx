...
`Image` is a drop-in replacement for `<img>` that optimizes on demand. It generates a resized version per device, serves WebP when the browser supports it (AVIF too, if you add it to `images.formats`), and lazy-loads anything below the fold automatically.
