...
Turbopack is the Rust-based, incremental, now-default bundler for `next dev` and `next build` — faster cold starts and HMR, `--webpack` to opt out, and a filesystem cache that is stable in dev and experimental in build.
</Callout>
